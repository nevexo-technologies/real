...
export default async function getValidationSchema(baseUrl = '') {
    ...
    return yup.object({
        ...
        age: yup.number().integer().min(26, "Vârsta minimă este de 20 de ani.").max(90, "Vârsta maxima este de 26 de ani.").required("Te rugăm să îți introduci vârsta."),
        ...
        t13: yup.number().integer().min(-1).max(1).required("Vă rugăm să alegeți un răspuns."),
        t13b: yup.number().integer().min(50).max(1000).when("t13", {
            is: 1,
            then: (schema) => schema.required("Vă rugăm să alegeți un răspuns.").typeError("Vă rugăm să alegeți un răspuns."),
            otherwise: (schema) => schema.nullable()
        }),
        t14: yup.number().integer().min(100).max(1000).required("Vă rugăm să alegeți un răspuns."),
        t15: yup.number().integer().min(50).max(1000).required("Vă rugăm să alegeți un răspuns."),
